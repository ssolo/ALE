...
void exODT_model::construct_undated(string Sstring) {
  ...
  S = TreeTemplateTools::parenthesisToTree(
      ...
      true //  (string_parameter["BOOT_STRAP_LABLES"]=="yes")
  );
      ...
      for (int e = 0; e < last_branch; e++) {
        ...
        if (not is_a_leaf) {
          ...
          for (int i = 0; i < N_parts; i++) {
            ...
            // T event
            uq_sum +=
                (uq[gp_i][e] * mPTuq[gpp_i] + uq[gpp_i][e] * mPTuq[gp_i]) * pp;
          }
        }
        if (not(e < last_leaf)) {
          int f = daughter[e];
...
string exODT_model::sample_undated(int e, int i, string last_event,
  ...
  if (not is_a_leaf) {
    ...
    for (int i = 0; i < N_parts; i++) {
      ...
      for (int f = 0; f < last_branch; f++) {
        uq_sum +=
            uq[gp_i][e] * (PT[f] / (float)last_branch) * uq[gpp_i][f] * pp +
            EPSILON;
        uq_sum +=
            uq[gpp_i][e] * (PT[f] / (float)last_branch) * uq[gp_i][f] * pp +
            EPSILON;
      }
    }
  }
    ...
    for (int i = 0; i < N_parts; i++) {
      ...
      if (not(e < last_leaf)) {
        ...
        if (r * uq_sum < uq_resum) {
          ...
          return "(" + sample_undated(f, gp_i, "S") + "," +
                 sample_undated(g, gpp_i, "S") + ")." + estr + branch_string +
                 ":" + branch_length;
        }
        uq_resum += PS[e] * uq[gp_i][g] * uq[gpp_i][f] * pp + EPSILON;
        if (r * uq_sum < uq_resum) {
          register_Su(e, last_event);
          return "(" + sample_undated(g, gp_i, "S") + "," +
                 sample_undated(f, gpp_i, "S") + ")." + estr + branch_string +
                 ":" + branch_length;
        }
      }
      // D event
      ...
      if (r * uq_sum < uq_resum) {
        register_D(e);
        return "(" + sample_undated(e, gp_i, "D") + "," +
               sample_undated(e, gpp_i, "D") + ").D@" + estr + branch_string +
               ":" + branch_length;
      }
      ...
      for (int f = 0; f < last_branch; f++) {
        ...
        if (r * uq_sum < uq_resum) {
          ...
          return "(" + sample_undated(e, gp_i, "S") + "," +
                 sample_undated(f, gpp_i, "T") + ").T@" + estr + "->" + fstr +
        ...
        if (r * uq_sum < uq_resum) {
          ...
          Ttokens.push_back(Ttoken.str());
          return "(" + sample_undated(e, gpp_i, "S") + "," +
                 sample_undated(f, gp_i, "T") + ").T@" + estr + "->" + fstr +
                 branch_string + ":" + branch_length;
        }
      }
    }
