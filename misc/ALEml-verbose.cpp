...
public:
  ...
  void fireParameterChanged(const ParameterList &pl) {
    ...
    cout << endl
         << "delta=" << delta << "\t tau=" << tau
         << "\t lambda=" << lambda //<< "\t lambda="<<sigma_hat << "\t ll="
         << -y << endl;
    fval_ = y;
  }
};
    ...
    for (int i = 0; i < (int)(*it).second.size(); i++)
      {
        ...
        if (branch==model->alpha)
          ...
          named_branch<<model->id_ranks[branch];

        vout << "\t" << model->gid_events[g_id][i] << "\t" <<
    model->gid_times[g_id][i]<< "\t" << named_branch.str() << "\t" <<
    model->gid_gidp[g_id][i] << "\t" << model->gid_gidpp[g_id][i] << "\t"
    <<endl;
      }
    */
  };
