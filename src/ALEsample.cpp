...
int main(int argc, char **argv) {
  ...
  if (argc < 3) {
    cout << "usage:\n ./ALEsample species_tree.newick gene_tree_sample.ale "
            "[samples=1000] [burnin=100]"
         << endl;
    return 1;
  }
  ...
  // p(ale) calculates the probability of the obsorved set of gene trees, i.e.
  // Pi(Gamma) cf. ALEPAPER
  ...
  rate_out << "#ALEsample using ALE v" << ALE_VERSION
  ...
  event_out << "#ALEsample using ALE v" << ALE_VERSION
            << " by Szollosi GJ et al.; ssolo@elte.hu; CC BY-SA 3.0;" << endl;
  event_out << "#"
            << "subsample"
            << "\t"
  ...
  sample_out << "#ALEsample using ALE v" << ALE_VERSION
             << " by Szollosi GJ et al.; ssolo@elte.hu; CC BY-SA 3.0;" << endl;
  ...
  while (sampled < N_samples) {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      scalar_type d;
      if (r < 1. / 3.)
        d = RandomTools::randExponential(0.001) * 2 *
            (0.5 - RandomTools::giveRandomNumberBetweenZeroAndEntry(1));
      else if (r < 2. / 3.)
        d = RandomTools::randExponential(0.01) * 2 *
            (0.5 - RandomTools::giveRandomNumberBetweenZeroAndEntry(1));
      else
        d = RandomTools::randExponential(0.1) * 2 *
            (0.5 - RandomTools::giveRandomNumberBetweenZeroAndEntry(1));
      ds.push_back(d);
    }
    ...
    if (new_p >= old_p or
        new_p / old_p > RandomTools::giveRandomNumberBetweenZeroAndEntry(1)) {
      old_p = new_p;
      delta = new_delta;
      tau = new_tau;
      lambda = new_lambda;
    ...
    if ((accapted > burnin and steps % print_mod == 0) or allprint) {
      ...
      rate_out << sampled << "\t" << steps << "\t" << delta << "\t" << tau
               << "\t" << lambda << "\t" << log(old_p) << endl;
      ...
      for (int i = 0; i < subsamples; i++) {
        ...
        event_out << i << "\t" << sampled << "\t" << steps << "\t"
                  << model->MLRec_events["D"] << "\t"
                  << model->MLRec_events["T"] << "\t"
