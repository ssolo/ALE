...
int main(int argc, char **argv) {
  ...
  for (int i = 2; i < argc; i++) {
    for (map<string, map<string, int>>::iterator kit = keep.begin();
         kit != keep.end(); kit++) {
      ...
      while (!file_stream1.eof()) {
        ...
        if (tree.find(")") != tree.npos) {
          ...
          tree_type *T =
              TreeTemplateTools::parenthesisToTree(tree, false, "ID");
          ...
          fout << TreeTemplateTools::treeToParenthesis(*T, false, "ID") << endl;
        }
      }
    }
  }
