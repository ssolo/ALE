...
int main(int argc, char **argv) {
  ...
  if (argc < 2) {
    cout << "usage:\n ./ALEestimate species_tree_file gene_tree_file "
  ...
  vector<string> gene_tree_strs; // Silly: we need to produce a vector with a
                                 // single element...
  gene_tree_strs.push_back(gene_tree_str);
  ale->observation(gene_tree_strs);
  cout << "\n\tObserved " << ale->observations
       << " gene tree(s) from: " << argv[2] << endl;
  ...
  for (int i = 3; i < argc; i++) {
    ...
    if (tokens[0] == "sample")
      ...
      if (tokens[1] == "y" || tokens[1] == "yes" || tokens[1] == "Y" ||
          tokens[1] == "YES") {
        outputyn = true;
      }
    }
  ...
  if (outputyn) {
    ...
    vector<string> tokens;
    boost::split(tokens, gene_tree_file, boost::is_any_of("/"),
                 boost::token_compress_on);
    ale_name = tokens[tokens.size() - 1];
    string outname = ale_name + ".uml_rec";
    ofstream fout(outname.c_str());
    fout << "#ALEevaluate using ALE v" << ALE_VERSION << "; CC BY-SA 3.0;"
         << endl
         << endl;
