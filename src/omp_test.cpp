...
int main(int argc, char **argv) {
  cout << "changing OMP_NUM_THREADS should leave the results unchanged!"
       << endl;
  ...
  model->calculate_EGb();
  // calculate joint ALE*exODT likelihood summed over all reconcilation and all
  // tree toplopgies

  // the openMP part should be added in model.cpp in exODT_model::p(
  // approx_posterior *) I guess you have to add the openMP bit into model.cpp
  // while making sure the above number does not change and also make sure that
  // the tables produed during the calculations work .. this can be done with
  // the stochastic backtrace, since we fixed the random seed, the above
  // reconciled tree should alos not change.. calling p(ale) after sample when
  // currrently unactivated pragma with label //p3 is active crashes
