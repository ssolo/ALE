...
exODT_sim::exODT_sim(int N_in, long int S_seed_in, scalar_type init_t_in,
                     scalar_type sigma_in) {
  if (S_seed_in == -1)
    S_seed = good_seed();
  else
    S_seed = S_seed_in;
...
string exODT_sim::sample_species(int n_in) {
  ...
  root_bl << age[lca]; // init_t-age[lca]; //more general printing like for
                       // genes would be nice..
  lca_age = age[lca];
...
vector<string> exODT_sim::simulate_gene_trees(
    int G_n, scalar_type delta, scalar_type tau, scalar_type lambda,
    scalar_type omega, bool only_root, long int G_seed_in, bool event_string) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (extant_species == population[sampled_i])
      for (vector<long int>::iterator git =
        ...
        extant_gene_name << i << "_"
                         << j; // extant_species << "_" << extant_gene;
