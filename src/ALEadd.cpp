...
int main(int argc, char **argv) {
  ...
  if (argc < 2) {
    cout << "usage:\n ./ALEadd ale_file.ale gene_tree_sample.newicks "
            "[weight=1] [burnin=0] [every=1] [until=end] [outfile=filename]"
         << endl;
    return 1;
  }
  ...
  cout << "# " << observe_trees.size() << " new tree(s) observed with weight "
       << weight << " from: " << argv[2];
  cout << "; " << burnin << " trees burnin discarded." << endl;

  cout << "# .ale with " << ale->observations << " tree(s) from: " << argv[1]
       << " and " << argv[2] << endl;
