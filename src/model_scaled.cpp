...
scalar_type exODT_model::p(approx_posterior *ale) {
  ...
  // We sort the directed partitions by size (number of gene tree leaves) to
  ...
  for (int g_id = -1; g_id < (int)g_ids.size(); g_id++) // ACCES par g_id+1
  {
    ...
    if (g_id == 0) { // n'existe pas --> case vide
           ...
           rank++) // Going through time slices, from leaves to root
      {
        ...
        vector<map<int, scalar_type>> vt_i;
        for (int t_i = 0;
             t_i < (int)time_slice_times[rank].size() + (rank == last_rank - 1);
...
//            if ( BipartitionTools::testBit ( temp, i) ) {
    ...
    for (int rank = 0; rank < last_rank; rank++) {
      ...
      for (int t_i = 0; t_i < (int)time_slice_times[rank].size(); t_i++) {
          ...
          for (int branch_i = 0; branch_i < n; branch_i++) {
            ...
            // boundary between slice rank and rank-1
              ...
              if (branch_i == n - 1 && rank > 0) {
                ...
                // SL EVENT, events #3 and #4 in part c of Fig.A1 in
                // http://arxiv.org/abs/1211.4606
                ...
                if (not is_a_leaf)
                  for (int i = 0; i < N_parts; i++) {
                    ...
                    // S EVENT, events #1 and #2 in part c of Fig.A1 in
                    // http://arxiv.org/abs/1211.4606
        ...
        if (1) {
          ...
          // note that the coalescent approximation in
          // http://arxiv.org/abs/1211.4606 is
          // exp(-(Delta_bar*(n-N)/N+Lambda_bar)*Delta_t );
          ...
          T_ap_epp=p_Ntau_e*qvec[gp_id+1][rank][t_i][alpha]*qvec[gpp_id+1][rank][t_i][e]*pp;
                    //T EVENT, events #3 and #4 in part b of Fig.A1 in
          http://arxiv.org/abs/1211.4606
          ...
          if (not is_a_leaf)
            for (int i = 0; i < N_parts; i++) {
              ...
              scalar_type Sb = 2 * sigma_hat * Delta_t *
                               ...
                               pp;
              // S_bar EVENT, event #2 in part b of Fig.A1 in
              // http://arxiv.org/abs/1211.4606 (note that Delta_bar corresponds
              //to sigma, the Delta_bar,Lambda_bar distinction keeps track of
              //speciaiton (birth) vs extiction (death),
          ...
          for (int branch_i = 0; branch_i < n; branch_i++) {
            ...
            // TL_bar EVENT, event #5 in part a of Fig.A1 in
            // http://arxiv.org/abs/1211.4606 (note that since Ebar ~ 1, most
            //transfers are expected to involve the TL evenet not the T event,
            // this should not be confused with the TL event of the
            // Tofigh/Doyon/ODTL models, which here corresponds
            //  to SL_bar + TL ..)
          ...
          // 0 EVENT, event #1 in part b of Fig.A1 in
          // http://arxiv.org/abs/1211.4606
          ...
          for (int branch_i = 0; branch_i < n; branch_i++) {
            ...
            if (not is_a_leaf)
              for (int i = 0; i < N_parts; i++) {
                ...
                // S_bar EVENT, events #3 and #4 in part a of Fig.A1 in
                // http://arxiv.org/abs/1211.4606 (The majority of transfer
                //events involve this event.)
                ...
                // D EVENT, event #2 in part a of Fig.A1 in
                // http://arxiv.org/abs/1211.4606
            ...
            // SL_bar EVENT, event #5 in part a of Fig.A1 in
            // http://arxiv.org/abs/1211.4606 (Transfer events where the donor
            //copy is lost involve this event.)
            ...
            // 0 EVENT, event #1 in part a of Fig.A1 in
            // http://arxiv.org/abs/1211.4606
            ...
            // if (qvec[g_id+1][tpdt_rank][tpdt_t_i][e]>1)
            // qvec[g_id+1][tpdt_rank][tpdt_t_i][e]=1; events within slice rank
            // at time t on branch e.
          }
        }
        // ######################################################################################################################
        // #########################################INNNER
  ...
  for (int rank = 0; rank < last_rank; rank++) {
    ...
    for (int t_i = 0; t_i < (int)time_slice_times[rank].size(); t_i++) {
      ...
      for (int branch_i = 0; branch_i < n; branch_i++) {
        int e = time_slices[rank][branch_i];
        // if (rank==last_rank-1 and
        // t_i==(int)time_slice_times[rank].size()-1)//(1-Ee[e][time_slice_times[rank][t_i]])/
        root_sum += qvec[0][rank][t_i][e] * Delta_t;
        survive += (1 - Ee[e][t]) * Delta_t;
        // cout << t<< " " <<rank << " " << branch_i << " " <<
        // qvec[0][rank][t_i][e]*Delta_t << " "<< (1-Ee[-1][t])*Delta_t <<endl;
      }
      // if (rank==last_rank-1 and
      // t_i==(int)time_slice_times[rank].size()-1)//(1-Ee[-1][time_slice_times[rank][t_i]]);
      root_sum += qvec[0][rank][t_i][alpha] * Delta_t;
      survive += Ee[-1][t] * Delta_t;
      // cout << t<< " " <<rank << " " << alpha << " " <<
      // qvec[0][rank][t_i][alpha]*Delta_t << " "<< (1-Ee[-1][t])*Delta_t
      // <<endl;
    }
  }
...
void exODT_model::calculate_EGb() {
  ...
  for (int rank = 0; rank < last_rank; rank++)
    for (int tsi = 0; tsi < (int)time_slice_times[rank].size(); tsi++) {
      ...
      scalar_type sigma_hat =
          scalar_parameter["sigma_hat"]; // vector_parameter["sigma_hat"][rank];//this
                                         // we usually set to 1
      scalar_type t = t_e;
      ...
      for (int ii = 0; ii < scalar_parameter["DD"]; ii++) {
        ...
        for (int i = 0; i < (int)time_slices[rank].size(); i++) {
          ...
          // k1 = f(y[n])
          E_k1[e] = delta * Ee_y[e] * Ee_y[e] * h +
                    (-sigma_hat * Ee_y[-1] - delta - lambda) * Ee_y[e] * h +
                    lambda * h;
        }
        // k2 = f(y[n]+h/2 k1)
