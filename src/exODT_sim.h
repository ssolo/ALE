...
public:
  ...
  exODT_sim(int N_in, long int S_seed_in = -1, scalar_type init_t_in = 2,
            scalar_type sigma = -1);

  std::string sample_species(int n_in);

  std::vector<std::string>
  simulate_gene_trees(int G_n, scalar_type delta, scalar_type tau,
                      scalar_type lambda, scalar_type omega = 0,
                      bool only_root = true, long int G_seed_in = -1,
                      bool event_string = false);

  ~exODT_sim() {
    population.clear();

    for (std::vector<std::vector<long int>>::iterator it = families.begin();
         it != families.end(); it++)
      (*it).clear();
    families.clear();
