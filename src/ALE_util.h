...
approx_posterior *observe_ALE_from_nexus(std::string fname, int burnin = 100,
                                         int every = 1, int until = -1);
approx_posterior *observe_ALE_from_file(std::string fname, int burnin = 100,
                                        int every = 1,
                                        int until = -1); // NO del-loc
approx_posterior *observe_ALE_from_file(std::vector<std::string> fnames,
                                        int burnin = 100, int every = 1,
                                        int until = -1); // NO del-loc
approx_posterior *
observe_ALE_from_strings(std::vector<std::string> trees);    // NO del-loc
