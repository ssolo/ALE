  ...
  for (int rank = 0; rank < last_rank; rank++)
    for (int tsi = 0; tsi < (int)time_slice_times[rank].size(); tsi++) {
      ...
      for (int ii = 0; ii < scalar_parameter["DD"]; ii++) {
        ...
        // E_k4[-1]); iy_E[-1][ii+1]=Ee_y[-1] + 1/6. * (E_k1[-1] + 2*E_k2[-1] +
        // 2*E_k3[-1] + E_k4[-1]);
        ...
        for (int i = 0; i < (int)time_slices[rank].size(); i++) {
          ...
          // E_k4[e]); iy_E[e][ii+1]=Ee_y[e] + 1/6. * (E_k1[e] + 2*E_k2[e] +
          // 2*E_k3[e] + E_k4[e]);
          ...
          // G_k4[e]); iy_G[e][ii+1]=Ge_y[e] + 1/6. * (G_k1[e] + 2*G_k2[e] +
          // 2*G_k3[e] + G_k4[e]);
