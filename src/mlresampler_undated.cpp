...
int main(int argc, char **argv) {
  ...
  cout << "LL: " << log(model->pun(ale)) << endl;
  cout << "time: " << t->elapsed() << endl;
  cout << ".." << endl;
  vector<Tree *> sample_trees;
  string outname = ml_rec_file + ".rate_resample.usamples";
  ofstream fout(outname.c_str());
  string outname2 = ml_rec_file + ".rate_resample.uTtokens";
  ...
  for (map<scalar_type, scalar_type>::iterator it = cum_copies.begin();
       it != cum_copies.end(); it++)
