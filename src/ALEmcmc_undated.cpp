 ...
 ************************************************************************/
// This version of the scaling function ensures that the new value is not larger
// than some maximum value
double scaleDoubleConstrained(const double &value, const double &maxi,
                              const double &lambda, double &hastingsRatio,
                              const bool verbose) {
  double newValue = value;
  // Generate new value (no reflection, so we simply abort later if we propose
  // value here outside of support)
 ...
 * mean:1/lambda
 ************************************************************************/
double computeExponentialLogProbability(const double &param,
                                        const double &value) {
...
double computeLogLk(exODT_model *model, approx_posterior *ale, const double &o,
                    const double &d, const double &t, const double &l) {
...
double computeLogPrior(const double &o, const double &d, const double &t,
                       const double &l, const double &priorOrigination,
                       const double &priorDelta, const double &priorTau,
                       const double &priorLambda) {
  double pp = 0.0;
...
void acceptMove(double &currentOrigination, double &currentDelta,
                double &currentTau, double &currentLambda,
                const double &newOrigination, const double &newDelta,
                const double &newTau, const double &newLambda) {
...
void rejectMove(const double &currentOrigination, const double &currentDelta,
                const double &currentTau, const double &currentLambda,
                double &newOrigination, double &newDelta, double &newTau,
                double &newLambda) {
...
void sampleTree(exODT_model *model, approx_posterior *ale, double &o, double &d,
                double &t, double &l, vector<string> &sample_strings,
...
int main(int argc, char **argv) {
  ...
  if (argc < 3) {
    cout << "\nUsage:\n ./ALEmcmc_undated species_tree.newick "
            "gene_tree_sample.ale sample=number_of_samples "
            "separators=gene_name_separator O_R=OriginationAtRootPrior "
            "delta=DuplicationRatePrior tau=TransferRatePrior "
            "lambda=LossRatePrior sampling_rate=sampling_rate "
            "beta=weight_of_sequence_evidence "
            "fraction_missing=file_with_fraction_of_missing_genes_per_species "
            "output_species_tree=n"
         << endl;
    cout << "\nExample:\n ./ALEmcmc_undated species_tree.newick "
            "gene_tree_sample.ale sample=100 separators=_ O_R=1 delta=0.01 "
            "tau=0.01 lambda=0.1 sampling_rate=10 beta=1\n"
         << endl;
    cout << "\n2nd example: we provide a file giving the expected fraction of "
         ...
         << endl;
    cout << "\n3rd example: same as 2nd, but outputs the annotated species "
            "tree to a file \n ./ALEmcmc_undated species_tree.newick "
            "gene_tree_sample.ale sample=100 separators=_ "
            "fraction_missing=fraction_missing.txt output_species_tree=y\n"
         << endl;
    cout << "\n4th example: use species tree branch lengths as fixed rate "
         ...
         << endl;
    cout << "\n5.1th example: use fixed branchrate multiplier for rate of Ts "
            "_to_ branch 43 with value 0.0 (i.e. no transfer to branch)\n "
            "./ALEmcmc_undated species_tree.newick gene_tree_sample.ale "
            "rate_mutiplier:tau_to:43:0.0 \n"
         << endl;
    cout << "\n5.2th example: use fixed branchrate multiplier for rate of Ts "
            "_from_ branch 43 with value 0.0 (i.e no transfer from branch)\n "
            "./ALEmcmc_undated species_tree.newick gene_tree_sample.ale "
            "rate_mutiplier:tau_from:43:0.0 \n"
         << endl;
    cout << "\n5.3th example: use fixed branchrate multiplier for rate Ds on "
            "branch 43 with value 0.0 (no duplications on branch)\n "
            "./ALEmcmc_undated species_tree.newick gene_tree_sample.ale "
            "rate_mutiplier:delta:43:0.0 \n"
         << endl;
    cout << "\n5.4th example: use fixed branchrate multiplier for rate Ls on "
            "branch 43 with value 0.0 (no losses on branch)\n "
            "./ALEmcmc_undated species_tree.newick gene_tree_sample.ale "
            "rate_mutiplier:lambda:43:0.0 \n"
         << endl;
  ...
  double priorOrigination = 1.0, priorDelta = 0.01, priorTau = 0.01,
         priorLambda = 0.1;
  ...
  double currentLogLikelihood = computeLogLk(
      model, ale, currentOrigination, currentDelta, currentTau, currentLambda);
  double newLogLikelihood = currentLogLikelihood;

  double currentLogPrior = computeLogPrior(
      currentOrigination, currentDelta, currentTau, currentLambda,
      priorOrigination, priorDelta, priorTau, priorLambda);
  double newLogPrior = currentLogPrior;

  std::cout << "Initial logLK: " << currentLogLikelihood
            << " and logPrior: " << currentLogPrior << std::endl;

  cout << "Reconciliation model initialised, starting DTL rate sampling"
       << ".." << endl;
  ...
  std::vector<int> orderScaleMoveParameters =
      VectorTools::seq(0, (int)scaleMoveParameters.size(), 1);
  ...
  for (i = 0; i < burninLength; ++i) {
    ...
    scale = scaleMoveParameters[RandomTools::pickOne(orderScaleMoveParameters,
                                                     scaleWeights, true)];
    if (move == originationId) {
      newOrigination = scaleDoubleConstrained(
          currentOrigination, maxOrigination, scale, hastingsRatio, verbose);
    } else if (move == deltaId) {
      newDelta = scaleDoubleConstrained(currentDelta,
                                        maxSumDTL - currentLambda - currentTau,
                                        scale, hastingsRatio, verbose);
    } else if (move == lambdaId) {
      newLambda = scaleDoubleConstrained(currentLambda,
                                         maxSumDTL - currentDelta - currentTau,
                                         scale, hastingsRatio, verbose);
    } else if (move == tauId) {
      newTau = scaleDoubleConstrained(currentTau,
                                      maxSumDTL - currentLambda - currentDelta,
                                      scale, hastingsRatio, verbose);
    }
    newLogLikelihood =
        computeLogLk(model, ale, newOrigination, newDelta, newTau, newLambda);
    newLogPrior =
        computeLogPrior(newOrigination, newDelta, newTau, newLambda,
                        priorOrigination, priorDelta, priorTau, priorLambda);
    // Accept or reject?
    acceptanceProbability = exp((newLogLikelihood + newLogPrior) -
                                (currentLogLikelihood + currentLogPrior)) *
                            hastingsRatio;
    threshold = RandomTools::giveRandomNumberBetweenZeroAndEntry(1.0);
    if (acceptanceProbability > threshold) { // accept
      acceptMove(currentOrigination, currentDelta, currentTau, currentLambda,
                 newOrigination, newDelta, newTau, newLambda);
      currentLogLikelihood = newLogLikelihood;
      currentLogPrior = newLogPrior;
    } else {
      rejectMove(currentOrigination, currentDelta, currentTau, currentLambda,
                 newOrigination, newDelta, newTau, newLambda);
    }
    std::cout << i << "\t" << currentLogLikelihood << "\t" << currentLogPrior
              << "\t" << currentOrigination << "\t" << currentDelta << "\t"
              << currentTau << "\t" << currentLambda << std::endl;
  }

  // MCMC loop
  std::cout << "MCMC during " << samples * sampling_rate << " iterations."
            << std::endl;
  std::cout << "LogLk"
            << "\t"
            << "LogPrior"
  ...
  for (i = 0; i < samples * sampling_rate; ++i) {
    ...
    if (i % sampling_rate == 0) {
      ...
      sampleTree(model, ale, currentOrigination, currentDelta, currentTau,
                 currentLambda, sample_strings, sample_trees);
    ...
    mcmcout << i << "\t" << currentLogLikelihood << "\t" << currentLogPrior
            << "\t" << currentOrigination << "\t" << currentDelta << "\t"
            << currentTau << "\t" << currentLambda << std::endl;
  }
  mcmcout.close();
  ...
  fout << "#ALEmcmc_undated using ALE v" << ALE_VERSION
  ...
  fout << "Total \t" << numDuplications / samples << "\t"
       << numTransfers / samples << "\t" << numLosses / samples << "\t"
       << numSpeciations / samples << endl;
  fout << endl;
  fout << "# of\t "
  ...
  if (ale->last_leafset_id > 3) {
    ...
    con_out << "#ALEmcmc_undated using ALE v" << ALE_VERSION
            << " by Szollosi GJ et al.; ssolo@elte.hu; CC BY-SA 3.0;" << endl;
