...
int main(int argc, char **argv) {
  ...
  // simulate
  cout << " First run: \n ./simulation N n" << endl;
  cout << "  In this command line, N corresponds to the total number of "
          "species, including extinct ones; "
       << endl;
  cout << "  n corresponds to the number of extant species only. " << endl;
  cout << "  This will produce a species tree in two files S_XXX.tree and "
          "R_XXX.tree. "
       << endl;
  cout << "  In the two files the tree is the same, but in R_XXX.tree the "
          "leaves have been renamed. "
       << endl;
  cout << "  XXX here corresponds to the species seed. " << endl;
  cout << "  if you like the species tree then remember the species seed "
          "S_seed and run: "
       << endl;
  cout << " ./simulate  N n S_seed  omega delta tau lambda " << endl;
  cout << " where omega delta tau lambda are the origination at the root, "
          "duplication, transfer and loss rate parameters."
       << endl;
  ...
  while (1) {
    ...
    int birth = death;
    while (birth == death)
      birth = RandomTools::giveIntRandomNumberBetweenZeroAndEntry(N);
  ...
  long int lca;
  int rank = 0;
  for (vector<vector<long int>>::reverse_iterator event = families.rbegin();
       event != families.rend(); event++) {
    scalar_type t_event = event_times[species_event - 1];
    long int mother = (*event)[0];
    long int daugther = (*event)[1];
    long int son = (*event)[2];
    if (sampled_population_counts[daugther] == 1 and
        sampled_population_counts[son] == 1) {
      rank++;
      ...
      strings[mother] = "(" + strings[daugther] + ":" + daugthers_bl.str() +
                        "," + strings[son] + ":" + sons_bl.str() + ")" +
                        rank_bs.str();
      age[mother] = t_event;
      lca = mother;
  ...
  root_bl << age[lca]; // init_t-age[lca]; //more general printing anyway needed
                       // for genes..
  ...
  while (species_event < number_of_species_events) {
    ...
    if (t - t_next > t_species_event) {
      ...
      scalar_type r =
          RandomTools::giveRandomNumberBetweenZeroAndEntry(rate_sum);
      if (r < gene_count * delta + gene_count * tau + gene_count * lambda) {
        int gene = -1;
        int species = -1;

        long int gene_r =
            RandomTools::giveIntRandomNumberBetweenZeroAndEntry(gene_count);
        long int gene_re_count = 0;
        gene_re_count = 0;
        for (int i = 0; i < N; i++) {
          if (gene_r <
              gene_re_count + (long int)population_of_genes[i].size()) {
            gene = (gene_re_count + (long int)population_of_genes[i].size() -
                    gene_r) -
                   1;
            species = i;
            break;
        ...
        // T
        {
          ...
          int T_to = species;
          while (T_to == species)
            T_to = RandomTools::giveIntRandomNumberBetweenZeroAndEntry(N);
          vector<long int> family;
          long int father = population_of_genes[species][gene];
          family.push_back(father);
          long int daugther = next_gene;
        ...
        // L
        {
          // cout << "L"<<endl;
          Ls += 1;
          int dj = RandomTools::giveIntRandomNumberBetweenZeroAndEntry(
              population_of_genes[species].size());
          population_of_genes[species].erase(
              population_of_genes[species].begin() +
              dj); // XX is this what I wanted?
          gene_count--;
    ...
    // S
    {
      ...
      for (vector<long int>::iterator gene =
               population_of_genes[births[species_event]].begin();
           gene != population_of_genes[births[species_event]].end(); gene++) {
        vector<long int> family;
  ...
  cout << "#gene simulation ends " << Ds << " Ds; " << Ts << " Ts; " << Ls
       << " Ls; " << Os << " Os; " << Ss << " Ss." << endl;
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (extant_species == population[sampled_i])
      for (vector<long int>::iterator git =
               population_of_genes[sampled_i].begin();
           git != population_of_genes[sampled_i].end(); git++) {
        long int extant_gene = (*git);
        stringstream extant_gene_name;
        extant_gene_name << strings[extant_species] << "_" << extant_gene;
        // extant_gene_name << sampled_i << "_" << j;// extant_species << "_" <<
        // extant_gene;
  ...
  for (vector<vector<vector<long int>>>::reverse_iterator event_vec =
           gene_families.rbegin();
       event_vec != gene_families.rend(); event_vec++) {

    for (vector<vector<long int>>::iterator event = (*event_vec).begin();
         event != (*event_vec).end(); event++) {
      ...
      if (sampled_gene_counts[daugther] == 1 and
          sampled_gene_counts[son] == 1) {
        gene_rank++;
        ...
        gene_rank_bs << gene_event_types[father] << suffix[son]
                     << suffix[daugther];
        suffix[father] = "";
        sons_bl << t_event - gene_age[son];
        daugthers_bl << t_event - gene_age[daugther];
        gene_strings[father] = "(" + gene_strings[daugther] + ":" +
                               daugthers_bl.str() + "," + gene_strings[son] +
                               ":" + sons_bl.str() + ")" + gene_rank_bs.str();
  ...
  int i = 0;
  for (map<long int, int>::iterator git = sampled_gene_counts.begin();
       git != sampled_gene_counts.end(); git++)
    if ((*git).second == 1 and
        gene_strings[(*git).first].find("(") != string::npos) {
      stringstream fname;
