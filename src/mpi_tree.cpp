...
void mpi_tree::load_distributed_ales(string fname)
{
  ...
  if (rank==server)
  {
    cout << "Loading ALEs from file " << fname << endl;
    if (!fexists(fname)) {
    ...
    if (file_stream.is_open())  //  ########## read trees ############
    {
      while (! file_stream.eof())
      {
        ...
        boost::split(tokens,line,boost::is_any_of("\t "),boost::token_compress_on);
        if (tokens.size() != 2) {
          cout << "Error, file "<< fname << " does not seem well formatted." << endl;
          cout << "Expecting only lines containing: ale_nameTABULATIONclient_index" << endl;
          cout << "Where ale_name is the name of an ale file, TABULATION is the tab character, and client_index is the index of the client in charge of this ale, starting at 0." << endl;
          exit(1);
        }
        int client_i=atoi(tokens[1].c_str());
        string ale_file=tokens[0];
        cout << "ALE file "<< ale_file << " has been allocated to client " << client_i << endl;
        if (ale_file.size()>1)
        {
...
void mpi_tree::estimate_rates()
{
  ...
  if (rank==server)
  {
    ...
    for (int e=0;e<model->last_branch;e++)
    {
      ...
      //cout << Ee << " " << Ge << endl;
      scalar_type P_D=max((scalar_type)1e-6,-1*((1 - Ee - Ge)/((-1 + Ee)*(-2*Ee + Ge + Ee*Ge))));
      scalar_type P_L=max((scalar_type)1e-6,-1*((Ee - 3*Ee*Ee + Ee*Ee*Ge)/((-1 + Ee)*(-2*Ee + Ge + Ee*Ge))));
      P_D=max((scalar_type)1e-6,model->branch_counts["Ds"][e]/N_S); // this works much better empirically
      scalar_type P_T=max((scalar_type)1e-6,model->branch_counts["Ts"][e]/Csum*(float)model->last_branch);
