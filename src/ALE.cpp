...
void approx_posterior::construct(string tree_string) {
  ...
  if (tree_string.substr(0, 1) != "(") {
    ...
    boost::split(leaves, tree_string, boost::is_any_of(","),
                 boost::token_compress_on);
  } else {
    tree_type *tree =
        TreeTemplateTools::parenthesisToTree(tree_string, false); // del-loc
...
void approx_posterior::save_state(string fname) {
  ...
  for (auto it = set_ids.begin(); it != set_ids.end(); it++) {
    ...
    /*   for (set< int>::iterator
       jt=(*it).first.begin();jt!=(*it).first.end();jt++) fout << "\t" <<
       (*jt);*/
    fout << endl;
  }
...
void approx_posterior::load_state(string fname) {
  ...
  /*    for ( auto it = size_ordered_bips.begin(); it !=
     size_ordered_bips.end(); it++ )
      {
...
scalar_type approx_posterior::Bi(int n2) const {
  ...
  n1 = max(2, n1);
  n2 = max(2, n2);
  return boost::math::double_factorial<scalar_type>(2 * n1 - 3) *
         boost::math::double_factorial<scalar_type>(2 * n2 - 3);
}
...
scalar_type approx_posterior::Tri(int n2, int n3) const {
  ...
  return boost::math::double_factorial<scalar_type>(2 * n1 - 3) *
         boost::math::double_factorial<scalar_type>(2 * n2 - 3) *
         boost::math::double_factorial<scalar_type>(2 * n3 - 3);
}
...
scalar_type approx_posterior::p_dip(set<int> gamma,set<int> gammap,set<int>
gammapp)
{
  if (Gamma_size<4)
    return 1;
...
scalar_type approx_posterior::p_dip(boost::dynamic_bitset<> gamma,
  ...
  // std::cout << "g_id: "<< g_id << " AND "<< gp_id << " AND "<< gpp_id << "
  // TO: "<< p_dip(g_id, gp_id, gpp_id) <<" " << p_dip( g_id, gpp_id, gp_id)
  // <<std::endl;
  if (gpp_id > gp_id)
    return p_dip(g_id, gp_id, gpp_id);
  else
...
scalar_type approx_posterior::p_bip(long int g_id) const {
  ...
  // if ( gamma.size()==1 or (int)gamma.size()==Gamma_size-1)
  // Bip_count=observations;
  if (set_sizes.count(g_id) == 0 or g_id == -10)
    Bip_count = 0;
  else if (set_sizes.at(g_id) == 1 or set_sizes.at(g_id) == Gamma_size - 1)
    Bip_count = observations;

  if (alpha > 0)
    return Bip_count / (observations + alpha) +
           (alpha / N_Gamma * Bi(set_sizes.at(g_id))) / (observations + alpha);
  else
    return Bip_count / observations;
}

scalar_type approx_posterior::p_dip(long int g_id, long int gp_id,
                                    long int gpp_id) const {
  if (Gamma_size < 4)
    return 1;
  ...
  if (Bip_counts.count(g_id) == 0 or g_id == -10 or !g_id) {
    ...
    if (gp_id == -10 or gpp_id == -10 or
        Dip_counts.at(g_id).count(parts) == 0 or !gp_id or !gpp_id) {
  ...
  if (set_sizes.count(g_id) == 0 or set_sizes.at(g_id) == 1 or
      set_sizes.at(g_id) == Gamma_size - 1)
    Bip_count = observations;
  // ? above is correct or not ?
  // cout << "Dip:"<<Dip_count << "  Bip:" << Bip_count << endl;
  if (alpha > 0 or beta > 0)
    return (Dip_count +
            (alpha / N_Gamma * Tri(set_sizes.at(gp_id), set_sizes.at(gpp_id))) +
            beta_switch * beta / (pow(2., set_sizes.at(g_id) - 1) - 1)) /
           (Bip_count + (alpha / N_Gamma * Bi(set_sizes.at(g_id))) +
            beta_switch * beta);
  else
...
map<boost::dynamic_bitset<>, scalar_type>
approx_posterior::recompose(string G_string) const {
  map<boost::dynamic_bitset<>, scalar_type> return_map;
  map<dedge_type, int> dedges;          // del-loc
  map<Node *, vector<Node *>> neighbor; // del-loc

  tree_type *G =
      TreeTemplateTools::parenthesisToTree(G_string, false); // del-loc
  ...
  for (vector<Node *>::iterator it = nodes.begin(); it != nodes.end(); it++) {
    ...
    if (!from->isLeaf()) {
      ...
      for (vector<Node *>::iterator it_sons = sons.begin();
           it_sons != sons.end(); it_sons++)
        neighbor[from].push_back((*it_sons));
      sons.clear();
    }
    for (vector<Node *>::iterator it_tos = neighbor[from].begin();
         it_tos != neighbor[from].end(); it_tos++) {
      dedge_type dedge;
      dedge.first = from;
      dedge.second = *it_tos;
      dedges[dedge] = 0;
  ...
  // Name all leaves
  for (map<dedge_type, int>::iterator it = dedges.begin(); it != dedges.end();
       it++) {
    ...
    if (from->isLeaf()) {
      ...
      // BipartitionTools::bit1(flat_names.at(dedge), static_cast<int>(
      // leaf_ids.at(from->getName() ) ) );
      ...
      // mark named
      dedges[dedge] = -1;
      // proceed to dedges in next level -  dedges from cherries can now be
      // named and at least one cherry must exist
      for (vector<Node *>::iterator it_tos = neighbor[to].begin();
           it_tos != neighbor[to].end(); it_tos++)
        if ((*it_tos) != from) {
  ...
  while (edges_left) {
    for (map<dedge_type, int>::iterator it = dedges.begin(); it != dedges.end();
      ...
      if (dedges[dedge] == 2) {
        ...
        //flat naming
        for (set<int>::iterator
        sit=leaf_set_in_1.begin();sit!=leaf_set_in_1.end();sit++)
          flat_names[dedge].insert((*sit));
        for (set<int>::iterator
        sit=leaf_set_in_2.begin();sit!=leaf_set_in_2.end();sit++)
          flat_names[dedge].insert((*sit));
      */
        /*
        ...
        //  BipartitionTools::bitOr(flat_names[dedge], leaf_set_in_1,
        //  leaf_set_in_2, nbint);
        ...
        // XX
        // cout << set2name(leaf_set_in_1) << " | "<< set2name(leaf_set_in_2) <<
        // endl; cout << q[dedges_in[0]] << " " << q[dedges_in[1]]<< " " <<
        // p_dip(flat_names[dedge],leaf_set_in_1,leaf_set_in_2) << endl;
        q[dedge] = q[dedges_in[0]] * q[dedges_in[1]] *
                   p_dip(flat_names[dedge], leaf_set_in_1, leaf_set_in_2);
        return_map[flat_names[dedge]] = q[dedge];
  ...
  // del-locs
  dedges.clear();
  for (map<Node *, vector<Node *>>::iterator it = neighbor.begin();
       it != neighbor.end(); it++)
    (*it).second.clear();
  neighbor.clear();
  delete G;
...
void approx_posterior::decompose(string G_string, set<int> *bip_ids,
  ...
  for (map<dedge_type, int>::iterator it = dedges.begin(); it != dedges.end();
    ...
    if (from->isLeaf()) {
      ...
      /* if (flat_names.find (dedge) == flat_names.end() ) {
       ...
       BipartitionTools::bit1(flat_names[dedge], static_cast<int>(
       leaf_ids[from->getName()] ) );
       */
  ...
  if (G->getLeaves().size() == 2) {
    ...
    while (edges_left) {
      for (map<dedge_type, int>::iterator it = dedges.begin();
           it != dedges.end(); it++) {
...
void approx_posterior::observation(vector<string> trees, bool count_topologies,
  ...
  set_sizes.clear();
  for (map<int, vector<long int>>::iterator it = size_ordered_bips.begin();
...
scalar_type approx_posterior::p(string tree_string) const {
  ...
  for (auto it = rec_map.begin(); it != rec_map.end(); it++) {
    ...
    if (std::isnan(p))
      p = 0; // NumConstants::VERY_TINY ();
    // std::cout << "rec_map[not_gamma]: "<<rec_map[not_gamma] <<" p_bip(gamma)
    // "<< p_bip(gamma) <<std::endl;
    break;
  }
  return p;
}
...
pair<string, scalar_type> approx_posterior::mpp_tree() const {
  map<long int, scalar_type> qmpp; // del-loc. Map between a bipartition id and
                                   // its maximum posterior probability.
  for (auto it = size_ordered_bips.begin(); it != size_ordered_bips.end(); it++)
    for (auto jt = (*it).second.begin(); jt != (*it).second.end(); jt++) {
      ...
      if ((*it).first == 1)
        ...
        for (auto kt = Dip_counts[g_id].begin(); kt != Dip_counts[g_id].end();
             kt++) {
          long int gp_id = (*kt).first.first;
          long int gpp_id = (*kt).first.second;
          scalar_type cp =
              p_dip(g_id, gp_id, gpp_id) * qmpp[gp_id] * qmpp[gpp_id];
          if (cp > max_cp)
            max_cp = cp;
        }
        qmpp[g_id] = max_cp;
  ...
  // Now we have maximum posterior probability estimates for all sets of leaves
  // (=bipartitions) The second loop computes the maximum posterior probability
  // estimate from all the trees that can be amalgamated (=all the trees that
  // can be written as the junction of two leaf sets)
  ...
  bs << max_pp / sum_pp * 2 << ":"
     << min(Bip_bls.at(max_bip) / Bip_counts.at(max_bip), (scalar_type)0.99);

  string max_tree = "(" + mpp_backtrack(max_bip, &qmpp) + "," +
                    mpp_backtrack(max_not_bip, &qmpp) + ")" + bs.str() + ";\n";
...
string approx_posterior::mpp_backtrack(long int g_id,
                                       map<long int, scalar_type> *qmpp) const {
  // leaf
  ...
  for (auto kt = Dip_counts.at(g_id).begin(); kt != Dip_counts.at(g_id).end();
       kt++) {
    long int gp_id = (*kt).first.first;
    long int gpp_id = (*kt).first.second;
    scalar_type cp =
        p_dip(g_id, gp_id, gpp_id) * (*qmpp).at(gp_id) * (*qmpp).at(gpp_id);
    sum_cp += cp;
    if (cp > max_cp) {
      max_cp = cp;
      max_gp_id = gp_id;
  ...
  return "(" + mpp_backtrack(max_gp_id, qmpp) + "," +
         mpp_backtrack(max_gpp_id, qmpp) + ")" + bs.str();
}
...
string approx_posterior::random_split(boost::dynamic_bitset<> gamma) const {
  ...
  //  for (set<int>::iterator sit=gamma.begin();sit!=gamma.end();sit++)
  //  gamma_v.push_back((*sit));
  for (auto i = 0; i < Gamma_size + 1; ++i) {
    if (gamma[i])
      gamma_v.push_back(i);
  }
  ...
  for (int gp_size = 1; gp_size <= (int)gamma_v.size() / 2; gp_size++) {
    ...
    if (g_id)
      for (auto dit = Dip_counts[g_id].begin(); dit != Dip_counts[g_id].end();
           dit++) {
    ...
    Bip_count = Bip_counts.at(g_id);
    if (gamma_size == 1 or gamma_size == Gamma_size - 1)
      Bip_count = observations;
    int nbip = binomial(gamma_size, gp_size);
    if (gamma_size - gp_size == gp_size)
      nbip /= 2;
    p_sum +=
        (0 + (alpha / N_Gamma * Tri(gp_size, gamma_size - gp_size)) +
         beta_switch * beta / (pow(2., (int)gamma_size - 1) - 1)) /
        (Bip_count + (alpha / N_Gamma * Bi(gamma_size)) + beta_switch * beta) *
        (nbip - saw);
    ...
    if (p_sum < 0) {
      ...
      while (!stop) {
        ...
        /* I propose a rewriting for that, using getSample instead of
         giveIntRandomNumberBetweenZeroAndEntry several times for (int
         i=0;i<gp_size;i++) gammap.insert(
         gamma_v[(RandomTools::giveIntRandomNumberBetweenZeroAndEntry(
         gamma_size ))] ); for (int i=0;i< gamma_size ;i++)  if
         (!gammap.count(gamma_v[i])) gammapp.insert(gamma_v[i]);*/
        ...
        if (Dip_counts.at(g_id).at(parts) == 0)
          stop = true;
      }
      break;
    }
...
vector<string>
approx_posterior::all_trees(boost::dynamic_bitset<> gamma) const {
  vector<string> all_trees_g;
  set<int> gamma_s; // not very efficient to use sets again, but this function
                    // is rarely used
  for (auto i = 0; i < Gamma_size + 1; ++i) {
  ...
  if (gamma_s.size() == 1) {
    ...
    for (set<set<int>>::iterator st = P_gamma.begin(); st != P_gamma.end();
         st++) {
      if (gamma_s.size() > (*st).size() and (*st).size() > 0 and
          (*st).count(*(gamma_s.begin())) == 1) {
        ...
        for (set<int>::iterator nst = gamma_s.begin(); nst != gamma_s.end();
             nst++)
          if ((*st).count(*nst) == 0)
            not_st.insert(*nst);
...
*/
        ...
        for (vector<string>::iterator lt = all_trees_gp.begin();
             lt != all_trees_gp.end(); lt++)
          for (vector<string>::iterator rt = all_trees_gpp.begin();
               rt != all_trees_gpp.end(); rt++) {
  ...
  if (Gamma_size == (int)gamma_s.size())
    for (vector<string>::iterator it = all_trees_g.begin();
         it != all_trees_g.end(); it++) {
      (*it) += ";\n";
    }
  return all_trees_g;
}
...
scalar_type
approx_posterior::count_all_trees(boost::dynamic_bitset<> gamma) const {
...
scalar_type approx_posterior::count_trees() const {
  ...
  for (auto it = size_ordered_bips.begin(); it != size_ordered_bips.end(); it++)
    for (auto jt = (*it).second.begin(); jt != (*it).second.end(); jt++) {
      ...
      if ((*it).first == 1)
        ...
        g_id_count[g_id] = 0;
        for (auto kt = Dip_counts.at(g_id).begin();
             kt != Dip_counts[g_id].end(); kt++) {
  ...
  for (auto it = Bip_counts.begin(); it != Bip_counts.end(); it++) {
    ...
    if (gamma_size > not_gamma_size)
      count_trees_g +=
          g_id_count[set_ids.at(gamma)] *
          g_id_count[set_ids.at(
              not_gamma)]; // count_trees(set_ids[gamma])*count_trees(set_ids[not_gamma]);
    else if (gamma_size == not_gamma_size)
      count_trees_g +=
          g_id_count[set_ids.at(gamma)] * g_id_count[set_ids.at(not_gamma)] /
          2.0; // count_trees(set_ids[gamma])*count_trees(set_ids[not_gamma])/2.0;
    // cout << count_trees(gamma) << " " << set2name(gamma) << " " <<
    // count_trees(not_gamma) << " " << set2name(not_gamma) <<endl;
  }
  cout << "..." << endl;
...
scalar_type approx_posterior::count_trees(long int g_id) const {
  ...
  if (gamma_size == 1) {
     ...
     */
        ...
        scalar_type count_trees_gpp =
            count_trees(set_ids.at(not_gammap)); // del-loc
        // not_gammap.clear();
...
scalar_type approx_posterior::nbipp(string tree_string) const {
  ...
  for (auto it = rec_map.begin(); it != rec_map.end(); it++) {
    boost::dynamic_bitset<> gamma = (*it).first;
    if (Bip_counts.at(set_ids.at(gamma)))
      n += 1;
    c += 1;
  }
  return n / c;
...
void approx_posterior::computeOrderedVectorOfClades(
    vector<long int> &ids, vector<long int> &id_sizes) {
  // I sort the directed partitions by size (number of gene tree leaves) to
  // ensure that we calculate things in the proper order (smaller to larger)
  for (map<int, vector<long int>>::iterator it = size_ordered_bips.begin();
       it != size_ordered_bips.end(); it++) {
