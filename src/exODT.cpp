...
exODT_model::exODT_model() {
  ...
  E_k4 = vector<scalar_type>(
      100, 0.0); // del-loc. Maps used for Runge-Kutta computations (4 stages).
...
double exODT_model::height(Node *node) {
  if (not node->isLeaf()) {
    ...
    return 0.5 * (sons[0]->getDistanceToFather() + height(sons[0]) +
                  sons[1]->getDistanceToFather() + height(sons[1]));
  } else
    return 0;
}

void exODT_model::construct(const string &Sstring, const scalar_type &N,
  ...
  S = TreeTemplateTools::parenthesisToTree(
      string_parameter["S_in"],
      (string_parameter["BOOTSTRAP_LABELS"] == "yes"));          // del-loc
  ...
  leaves.clear();
  for (map<string, Node *>::iterator it = leaf_sort.begin();
       it != leaf_sort.end(); it++)
    leaves.push_back((*it).second);
  leaf_sort.clear();
  // leaves is now sorted by alphabetical order of the names
  map<Node *, int>
      next_generation; // Map between node and slice id of descendant nodes.
  ...
  while (1) {
    vector<Node *> tmp;
    bool stop = true;
    for (map<Node *, int>::iterator it = next_generation.begin();
         it != next_generation.end(); it++)
      if (next_generation[(*it).first] ==
          -1) // father of leaves at first, then a node that needs to be
              // examined
      {
        Node *node = (*it).first;
  ...
  for (map<Node *, scalar_type>::iterator it = node_ts.begin();
       it != node_ts.end(); it++) {
    (*it).second /= h;
  ...
  for (map<Node *, scalar_type>::iterator it = node_ts.begin();
    ...
    // nonleaves
    if (t > 0) {
      // degenerate speciation times, where >1 nodes have same age .. should be
      // avoided!
  ...
  for (map<scalar_type, Node *>::iterator it = t_nodes.begin();
  ...
  for (map<scalar_type, Node *>::iterator it = t_nodes.begin();
       it != t_nodes.end();
       it++) { // We go through the nodes, ordered according to their age.
    scalar_type t = (*it).first;
    Node *node = (*it).second;
    ...
    // the root
      ...
      t_begin[last_branch] =
          t_end[last_branch] + scalar_parameter["stem_length"];
    last_rank++;
    last_branch++;
    sons.clear();
  }
  ...
  int i = 0;
  for (map<string, int>::iterator it = species_order.begin();
       it != species_order.end(); it++) {
    (*it).second = i;
  ...
  for (vector<Node *>::iterator it = nodes.begin(); it != nodes.end(); it++) {
    ...
    if (not(*it)->isLeaf()) {
      ...
      for (vector<Node *>::iterator jt = tmp.begin(); jt != tmp.end(); jt++) {
        // cout << (*jt)->getName() << ":" << species_order[ (*jt)->getName()
        // ]<<endl;
        tmp2[(*jt)->getName()] = -1;
      }
      for (map<string, int>::iterator jt = tmp2.begin(); jt != tmp2.end();
           jt++) {
  ...
  for (map<int, string>::iterator it = extant_species.begin();
  ...
  for (int rank = 0; rank < last_rank; rank++) {
    ...
    if (rank == 0) {
      ...
      for (vector<int>::iterator it = time_slices[rank - 1].begin();
           it != time_slices[rank - 1].end(); it++) {
        int branch = (*it);
  ...
  for (int rank = 0; rank < last_rank; rank++) {
    ...
    // cout << rank << " " << local_D << " " << slice_height<< " "
    // <<slice_height/delta_t << endl; we calculate the local D
    for (scalar_type internal_interval = 1; internal_interval < local_D;
         internal_interval++) {
      time_slice_times[rank].push_back(slice_end + internal_interval *
                                                       slice_height / local_D);
    }
    time_slice_begins[rank] = slice_begin;

    // for (scalar_type
    // internal_interval=1;internal_interval<scalar_parameter["D"];internal_interval++)
    //{
  ...
  // cf. section S1.4 of the Supporting Material of
  // www.pnas.org/cgi/doi/10.1073/pnas.1202997109 compatible with p(ale) not
  // compatible with sample() and p_MLRec(ale)
  ...
  // if we assume the that height of the species tree is equal to its expected
  // value under the colaescent
  //  cf. http://arxiv.org/abs/1211.4606
  // Delta is sigma, i.e. the speciation rate of the Moran model in
  // http://arxiv.org/abs/1211.4606 and ALEPAPER
  ...
  vector_parameter["fraction_missing"] =
      vector<scalar_type>(leaves.size(), 0.0);
...
void exODT_model::set_model_parameter(string name,
                                      vector<scalar_type> value_vector) {
  if (name == "delta" or name == "tau" or name == "lambda") {
    scalar_type N = vector_parameter["N"][0];
    vector_parameter[name].clear();
    scalar_type avg = 0;
