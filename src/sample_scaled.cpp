...
string exODT_model::sample(bool S_node, long int g_id, int t_i,
  ...
  if (rank > -1) {
    ...
    if (e == alpha) {
        ...
        if (not is_a_leaf)
          for (int i = 0; i < N_parts; i++) {
            ...
            scalar_type Sb = 2 * sigma_hat * Delta_t *
                             ...
                             pp;
            // cout << "Sb2:
            // "<<2*sigma_hat*Delta_t<<"*"<<qvec[gp_id+1][rank][t_i][alpha]<<"*"<<qvec[gpp_id+1][rank][t_i][alpha]<<"*"<<pp<<endl;
            // S_bar EVENT
            resum += Sb;
            if (1) {
          ...
          if (e == time_slices[rank][n - 1] && rank > 0) {
            ...
            if (not is_a_leaf)
              for (int i = 0; i < N_parts; i++) {
                ...
                // cout << rank<< " " << t_i << " " <<
                // time_slice_times[rank][t_i] <<  endl; S EVENT
      ...
      // boundaries for branch e.
        ...
        if (not is_a_leaf)
          for (int i = 0; i < N_parts; i++) {
            ...
            scalar_type Sb_pa_ppe = sigma_hat * Delta_t *
                                    qvec[gp_id + 1][rank][t_i][alpha] * qppe *
                                    pp;
            scalar_type Sb_pe_ppa = sigma_hat * Delta_t * qpe *
                                    qvec[gpp_id + 1][rank][t_i][alpha] * pp;
            // cout << "Sb: " <<
            // sigma_hat*Delta_t<<"*"<<qvec[gp_id+1][rank][t_i][alpha]<<"*"<<qppe<<"*"<<pp<<endl;
            // cout << "  : " <<
            // sigma_hat*Delta_t<<"*"<<qpe<<"*"<<qvec[gpp_id+1][rank][t_i][alpha]<<"*"<<pp<<endl;
        ...
        scalar_type empty =
            (1 + (2 * delta_e * Eet - sigma_hat * H_hat - delta_e - lambda_e) *
                     Delta_t) *
  ...
  /*
    ...
    " t_i:" << t_i << " " << time_slice_times[back_step.rank][t_i]<< endl; else
      cout  <<  back_step.event << "\t" << back_step.rank << "\tid_rank:" <<
    id_ranks[back_step.e] << "\text_sp:" << extant_species[back_step.e] <<
    "\te:" << back_step.e<< "\tg_id:" <<
    ale_pointer->set2name(ale_pointer->id_sets[g_id]) << "\t" << back_step.t <<
    " t_i:" << t_i  << " " << time_slice_times[back_step.rank][t_i]<<  endl;
  */
  stringstream toptmp;
  ...
  if (back_step.event == "D" or back_step.event == "Tb" or
    ...
    if (back_step.event == "S") {
      ...
      if (back_step.event == "Tb") {
        ...
        if (transfer_token != "")
          ...
          transfer_token_stream << "T|" << rank << "|" << t << "|"
                                << named_branch.str() << "|" << g_id;
    ...
    if (transfer_token_stream.str() == "")
      ...
      return "(" +
             sample(S_node, back_step.gp_id, t_i + (back_step.event == "S"),
                    rank, back_step.ep, 0, "", transfer_token_stream.str(),
                    max_rec) +
             "," +
             sample(S_node, back_step.gpp_id, t_i + (back_step.event == "S"),
                    rank, back_step.epp, 0, "", transfer_token, max_rec) +
      ...
      return "(" +
             ...
             "," +
             sample(S_node, back_step.gpp_id, t_i + (back_step.event == "S"),
                    rank, back_step.epp, 0, "", transfer_token_stream.str(),
                    max_rec) +
             ")" + branch_string.str();
    ...
    if (transfer_token_stream.str() == "") {
      return sample(S_node, back_step.g_id, t_i + (back_step.event == "SL"),
                    rank, back_step.e, new_branch_length, branch_string.str(),
