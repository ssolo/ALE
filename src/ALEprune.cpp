...
int prune(string *tree, vector<string> keep_list, vector<string> *keep_leaves) {
  ...
  for (vector<string>::iterator it = keep_list.begin(); it != keep_list.end();
       it++)
    keep_map[(*it)] = 1;
  ...
  for (vector<Node *>::iterator it = leaves.begin(); it != leaves.end(); it++) {
    ...
    string sp = tokens[0];
    // string sp=name;
    if (keep_map.count(sp) == 1)
      (*keep_leaves).push_back(name);
  ...
  if (N_leaves > 3)
    *tree = TreeTemplateTools::treeToParenthesis(*T, false, "ID");
  return N_leaves;
}
...
int cout_small(vector<string> keep_leaves, string ale_name) {
  ...
  if (keep_leaves.size() == 1) {
    ...
    fout << "#constructor_string" << endl;
    fout << keep_leaves[0] + "," + keep_leaves[1] + "," + keep_leaves[2]
         << endl;
    fout << "#observations" << endl;
    fout << "1" << endl;
    fout << "#Bip_counts" << endl;
...
int main(int argc, char **argv) {
  ...
  vector<string> tokens;
  boost::split(tokens, ale_name, boost::is_any_of("."),
  ...
  while (!ale_stream.eof()) {
    ...
    if (line[0] == '#')
      ...
      if (field == "#constructor_string") {
        ...
        boost::split(tokens, line, boost::is_any_of("\t "),
                     boost::token_compress_on);
        Dip_counts[atol(tokens[0].c_str())][atol(tokens[1].c_str())]
                  [atol(tokens[2].c_str())] = atof(tokens[3].c_str());
        ...
        for (vector<string>::iterator it = tokens.begin(); it != tokens.end();
          ...
          temp[static_cast<int>(atoi((*it).c_str()))] =
              1; // cout << id_leaves[static_cast<int>(atoi((*it).c_str()))] <<
                 // " ";
        }
  ...
  if (N_leaves < 4)
    return cout_small(keep_leaves, ale_name);
  ...
  for (vector<string>::iterator it = keep_leaves.begin();
       it != keep_leaves.end(); it++) {
    string name = (*it);
    ...
    cout << i << " " << name << " " << leaf_ids[name] << "  " << keep_set << " "
         << old_set << endl;
  ...
  for (map<boost::dynamic_bitset<>, long int>::iterator it = set_ids.begin();
       it != set_ids.end(); it++) {
    ...
    // if (old_set!=keep_set and not (new_set.none())) new_sets[old_set] =
    // new_set;

    if (old_set != keep_set and new_set != keep_set and not(new_set.none()) and
        (new_set_ids.count(new_set) == 0) and Bip_counts.count((*it).second))
      new_Bip_counts[new_set] = 0;
  }

  for (map<boost::dynamic_bitset<>, scalar_type>::iterator it =
           new_Bip_counts.begin();
       it != new_Bip_counts.end(); it++) {
    new_set_ids[(*it).first] = new_last_leafset_id;
    new_id_sets[new_last_leafset_id] = (*it).first;
    new_last_leafset_id++;
  }

  for (map<long int, map<long int, map<long int, scalar_type>>>::iterator it =
           Dip_counts.begin();
       it != Dip_counts.end(); it++) {
    boost::dynamic_bitset<> gamma = id_sets[(*it).first];
    for (map<long int, map<long int, scalar_type>>::iterator jt =
             (*it).second.begin();
         jt != (*it).second.end(); jt++) {
      boost::dynamic_bitset<> gamma_p = id_sets[(*jt).first];
      for (map<long int, scalar_type>::iterator kt = (*jt).second.begin();
           kt != (*jt).second.end(); kt++) {
        boost::dynamic_bitset<> gamma_pp = id_sets[(*kt).first];

        if (new_sets[gamma].none() or new_sets[gamma_p].none() or
            new_sets[gamma_pp].none() or (new_sets[gamma] == keep_set)) {
          ;
        } else {
          ...
          // cout << new_sets[gamma] << " " << new_sets[gamma_p] << " " <<
          // new_sets[gamma_pp] << endl; cout <<
          // "---------------------------------" << endl;
  ...
  fout << "#Bip_bls" << endl;
  for (map<boost::dynamic_bitset<>, long int>::iterator it =
           new_set_ids.begin();
       it != new_set_ids.end(); it++) {
  ...
  fout << "#Dip_counts" << endl;
  for (map<long int, map<long int, map<long int, scalar_type>>>::iterator it =
           new_Dip_counts.begin();
       it != new_Dip_counts.end(); it++) {
    ...
    for (map<long int, map<long int, scalar_type>>::iterator jt =
      ...
      for (map<long int, scalar_type>::iterator kt = (*jt).second.begin();
        ...
        fout << new_g_id << "\t" << new_gp_id << "\t" << new_gpp_id << "\t"
             << (*kt).second << endl;
      }
    }
  }
