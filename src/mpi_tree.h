...
public:
  ...
  std::vector<scalar_type> delta_branch_avg, tau_branch_avg,
      lambda_branch_avg; // del-loc
  std::vector<scalar_type> delta_branch_norm, tau_branch_norm,
      lambda_branch_norm; // del-loc
  std::string S_string;
  mpi_tree(std::string Sstring, const boost::mpi::communicator mpi_world,
           std::map<std::string, scalar_type> set_parameters =
               std::map<std::string, scalar_type>(),
           bool undated = false) {
    if (undated) {
      S_string = Sstring;
      ...
      for (std::map<std::string, scalar_type>::iterator it =
               set_parameters.begin();
           it != set_parameters.end(); it++)
      ...
      scalar_type N = 1e6;
      model->set_model_parameter(
          "N",
          1e6); // we can almost scale out N assuming height from coalescent..
  ...
  ~mpi_tree() {
    for (std::vector<approx_posterior *>::iterator it = ale_pointers.begin();
         it != ale_pointers.end(); it++)
      delete (*it);
    ale_pointers.clear();
    MLRec_res.clear();
    client_fnames.clear();
    delta_branch_avg.clear(), tau_branch_avg.clear(),
        lambda_branch_avg.clear(); // del-loc
    delta_branch_norm.clear(), tau_branch_norm.clear(),
        lambda_branch_norm.clear(); // del-loc
  ...
  scalar_type calculate_pun(int n = 0);
  scalar_type calculate_punt(std::string S);
  scalar_type calculate_pun(int n, bool bw);
