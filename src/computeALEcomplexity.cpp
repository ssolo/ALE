...
int main(int argc, char **argv) {
  ...
  if (argc < 5) {
    std::cout << "\n\n\t\tUsage: computeALEcomplexity speciesTreeFile ALEfile "
                 "delta tau lambda \n"
              << std::endl;
    cout << "The program outputs statistics on an ale file as "
            "follows:\nale_file \t loglk \t "
            "NumberOfGenes\tNumberOfSpecies\tComputingTime1\tComputingTime2\tCo"
            "mputingTime3\tAverageComputingTime"
         << endl;
    std::cout.flush();
    exit(1);
  }
  ...
  // it=simulation->gene_trees.begin();it!=simulation->gene_trees.end();it++)
  // cout << (*it) << endl;
  ...
  for (auto gene_name = leafNames.begin(); gene_name != leafNames.end();
       ++gene_name) {
  ...
  cout << ale_file << "\t" << ll << "\t" << leafNames.size() << "\t"
       << numSpecies << "\t" << time1 << "\t" << time2 << "\t" << time3 << "\t"
       << (time1 + time2 + time3) / 3 << endl;
  cout.flush();
