...
readFractionMissingFile(std::string fractionMissingFile) {
  std::map<std::string, scalar_type> toReturn;
  if (fractionMissingFile == "") {
    std::cout << "No file providing the fraction of missing genes per species, "
                 "we assume that all species have 100% of their genes."
              << std::endl;
  } else {
    if (!fexists(fractionMissingFile)) {
      std::cout << "Error, file " << fractionMissingFile
    ...
    for (std::vector<std::string>::iterator it = listCoverages.begin();
         it != listCoverages.end(); it++) {
